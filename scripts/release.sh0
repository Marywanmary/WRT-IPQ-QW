...
# 打包固件文件
if [ -d "$ARTIFACTS_DIR/firmware" ] && [ "$(ls -A $ARTIFACTS_DIR/firmware)" ]; then
    echo "打包固件文件..."
...
# 打包配置文件
if [ -d "$ARTIFACTS_DIR/config" ] && [ "$(ls -A $ARTIFACTS_DIR/config)" ]; then
    echo "打包配置文件..."
...
# 打包日志文件
if [ -d "$ARTIFACTS_DIR/log" ] && [ "$(ls -A $ARTIFACTS_DIR/log)" ]; then
    echo "打包日志文件..."
...
# 打包软件包
if [ -d "$ARTIFACTS_DIR/app/packages" ] && [ "$(ls -A $ARTIFACTS_DIR/app/packages)" ]; then
    echo "打包软件包..."
